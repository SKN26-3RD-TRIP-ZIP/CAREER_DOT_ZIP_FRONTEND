    ...
    <ReportLayout title="면접관 피드백" subtitle="면접 시 사용한 면접관 페르소나 기준으로 피드백을 제공합니다." action={back}>
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <div className="flex items-center gap-3">
              <h3 className="text-lg font-extrabold text-[#000000]">선택한 면접관: {f.persona.short_name}</h3>
              <Badge tone="soft">Total Score {f.persona.total_score}점</Badge>
            </div>
            <p className="mt-1 text-sm text-[rgba(0,0,0,0.5)]">실제 면접관처럼 {f.persona.short_name} 중심으로 봅니다.</p>
            <div className="mt-2 flex flex-wrap gap-2">
              {f.persona.tags.map((t) => <Tag key={t} label={t} variant="strength" />)}
            </div>
          </div>
        </div>
        <p className="max-w-xs text-sm text-[rgba(0,0,0,0.45)]">다른 면접관을 선택해 페르소나별 피드백/모의질문 차이를 비교해 보세요.</p>
      </section>

      <section className="mt-4 grid gap-4 lg:grid-cols-3">
        {/* 피드백 요약 */}
        <div className="rounded-xl border border-[rgba(0,0,0,0.1)] bg-[#EEEEEE] p-6 shadow-sm">
          <h3 className="mb-3 text-base font-bold text-[#000000]">피드백 요약</h3>
          <p className="text-sm leading-relaxed text-[rgba(0,0,0,0.6)]">{f.summary}</p>
          <div className="mt-4">
            <div className="mb-2 text-xs font-bold text-[rgba(0,0,0,0.5)]">추천 답변 구조</div>
            <div className="flex flex-wrap items-center gap-1.5 text-xs text-[rgba(0,0,0,0.5)]">
              {f.recommended_answer_structure.map((s, i) => (
                <span key={s} className="flex items-center gap-1.5">
                  <span className="rounded-md bg-[rgba(0,0,0,0.06)] px-2 py-1 font-medium text-[rgba(0,0,0,0.7)]">{s}</span>
                  {i < f.recommended_answer_structure.length - 1 && <span className="text-[rgba(0,0,0,0.3)]">→</span>}
                </span>
              ))}
            </div>
          </div>
        ...
        {/* 좋았던 점 */}
        <div className="rounded-xl border border-[rgba(0,0,0,0.1)] bg-[#EEEEEE] p-6 shadow-sm">
          <h3 className="mb-3 text-base font-bold text-[#000000]">좋았던 점</h3>
          <ul className="space-y-2.5">
            {f.pros.map((p, i) => (
              <li key={i} className="flex gap-2 text-sm text-[rgba(0,0,0,0.6)]">
        ...
        {/* 보완할 점 */}
        <div className="rounded-xl border border-[rgba(0,0,0,0.1)] bg-[#EEEEEE] p-6 shadow-sm">
          <h3 className="mb-3 text-base font-bold text-[#000000]">보완할 점</h3>
          <ul className="space-y-2.5">
            {f.cons.map((c, i) => (
              <li key={i} className="flex gap-2 text-sm text-[rgba(0,0,0,0.6)]">
                <span className="mt-1.5 h-1.5 w-1.5 shrink-0 rounded-full bg-[#000000]" />{c}
              </li>
            ))}
          </ul>
        </div>
      ...
      <section className="mt-4 grid gap-4 lg:grid-cols-3">
        ...
        <div className="rounded-xl border border-[rgba(0,0,0,0.1)] bg-[#EEEEEE] p-6 shadow-sm lg:col-span-2">
          <h3 className="mb-4 text-base font-bold text-[#000000]">{f.persona.short_name} 예상 질문</h3>
          <ul className="space-y-3">
            {f.expected_questions.map((q) => (
              <li key={q.order} className="flex gap-3 text-sm">
                <span className="h-fit shrink-0 rounded-md bg-[#253900] px-2 py-0.5 text-xs font-bold text-[#EEEEEE]">Q{q.order}</span>
                <span className="text-[rgba(0,0,0,0.6)]">{q.text}</span>
              </li>
            ))}
          </ul>
        </div>
        ...
        {/* 다음 연습으로 연결 */}
        <div className="flex flex-col justify-between rounded-xl border border-[rgba(0,0,0,0.1)] bg-[#EEEEEE] p-6 shadow-sm">
          <div>
