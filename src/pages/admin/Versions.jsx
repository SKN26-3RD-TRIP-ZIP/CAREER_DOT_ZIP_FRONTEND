  ...
  return (
    <div className="flex flex-col">
      ...
      <div className="mb-6">
        ...
        <p className="mt-0.5 text-xs text-slate-400">새로운 버전을 배포합니다.</p>
      </div>

      {/* 프롬프트 내용 */}
      <label className="mb-2 text-sm font-semibold text-slate-800">프롬프트 내용</label>
      <textarea
        className="mb-5 h-64 resize-none rounded-xl border border-slate-200 p-4 text-sm text-slate-700 outline-none placeholder:text-slate-300 focus:border-emerald-400"
      ...
      {/* 메모 */}
      <label className="mb-2 text-sm font-semibold text-slate-800">메모</label>
      <textarea
        className="mb-4 h-24 resize-none rounded-xl border border-slate-200 p-4 text-sm text-slate-700 outline-none placeholder:text-slate-300 focus:border-emerald-400"
  ...
  return (
    <div className="flex flex-col">
      ...
      <div className="mb-6 flex items-center justify-between">
        <h2 className="text-base font-semibold text-slate-900">버전 히스토리</h2>
        <button
          onClick={onDeploy}
      ...
      {isLoading ? (
        ...
        <p className="py-10 text-center text-sm text-slate-400">버전이 없습니다.</p>
      ) : (
            ...
            {versions.map((ver) => {
              ...
              return (
                <div key={ver.prompt_ver_id} className="relative flex items-start gap-4 py-4">
                  ...
                  {/* 내용 */}
                  <div className="flex flex-1 items-start justify-between gap-4 border-b border-slate-100 pb-4">
                    <div>
                      ...
                      {ver.change_note && (
                        <p className="mt-0.5 text-xs text-slate-500">{ver.change_note}</p>
                      )}
                    </div>
...
export default function Versions() {
  ...
  const { data: versions, isLoading: versionsLoading } = useQuery({
    ...
    select: (data) => data, // already an array from adminApi
  })
  ...
  return (
    <div className="flex flex-col gap-6">
      {/* 헤더 */}
      <div>
        <h1 className="text-2xl font-bold text-slate-900">버전 관리</h1>
