...
function JsonCell({ value }) {
  ...
  return (
    <button onClick={() => setOpen((v) => !v)} className="text-left text-xs text-slate-600">
  ...
  return (
    <div className="flex flex-col gap-4 p-2">
      ...
      {loading ? (
        ...
        <p className="py-12 text-center text-sm text-slate-400">감사 로그가 없습니다.</p>
      ) : (
            ...
            <tbody>
              {logs.map((log) => (
                <tr key={log.audit_log_id} className="border-t border-slate-100">
                  ...
                  <td className="px-3 py-2 font-medium text-slate-800">{log.action_type}</td>
